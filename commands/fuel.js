...
module.exports = {
    name: 'fuel',
    description: 'Start a fake duel between 2 users.',
    async execute(message, opponent) {
        let done = false;
        const embed = new MessageEmbed()
        .setTitle('Fake Duel Challenge ⚔️')
        .setColor('#dc322f')
        .setTimestamp()
        .setDescription(`${message.author} has challenged ${opponent} to a fake duel! 🔫\nType \`y\` to accept.`)
        ...
        //wait for accept
        const collector = new MessageCollector(message.channel, m => m.author.id === message.author.id && m.content.toLowerCase() == "y", { time: 60000 });
        collector.on('collect', accept => {
            ...
            const earlyCollector = new MessageCollector(message.channel, m => m.author.id === opponent.id || m.author.id === opponent.id, { time: 10000 });
            ...
            let time = getRandomInt(2000, 10000);
            let fakeTime = getRandomInt(1000, time-100);
            let fakeCount = getRandomInt(0, Math.floor(Math.floor(time/1000)/2));
            for(i = 0; i < fakeCount; i++) {
                setTimeout(() => {
                    if(!done) {
                        message.channel.send(fakeEmbed);
                    }
                }, fakeTime);
                fakeTime = getRandomInt(1000, time-100);
            }
            setTimeout(() => {
                if(!done) {
